...
void mainread(unsigned char *data,unsigned int lenght,unsigned int baddr,unsigned int paddr){  
 ...
 unsigned int i;      
 paddr<<=2;
 if(baddr&0x0100)paddr|=1;
 if(baddr&0x0200)paddr|=2;
 _CS=0;
 spi(0x52);
 addr=&paddr;
 spi(*(addr+1));
